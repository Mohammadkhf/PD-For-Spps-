function [ gap_function, x_bar, y_bar,iteration_time] = ALPD_quad_obj_quad_cons(n,m,K,Q,c,A,B,d,l_yy,l_xy,l_xx, l_f, l_g,mu_f,mu_g,D_x, D_y,x_init,y_init);
...
for t=1:K
    ...
    cvx_begin quiet  %optimizaing in dual 
        variable  y_sol(m)
        minimize (-y_sol'*v(:,t)+ y_sol'*d+ (0.5*mu_g)*sum_square_abs(y_sol) + tau(t)/2*sum_square_abs(y_sol-y(:,t))); %exact dual
        %minimize (-y_sol'*v(:,t)+ y_sol'*d+ (mu_g)*y_sol'*y(:,t) +
        %tau(t)/2*sum_square_abs(y_sol-y(:,t)));     approx dual
        subject to 
            0 <= y_sol;
            norm(y_sol)<= D_y;
            %norm(y_sol,1)<=D_y;
    ...
    cvx_end
        ...
        grad_y__x_t = tensor *x(:,t)+ B*y(:,t+1);
    cvx_begin quiet %optimizing in primal 
    cvx_precision best
    cvx_solver sedumi
        variable x_sol(n)
        minimize(x_sol'*Q*x_underbar(:,t)+c'*x_sol + x_sol'*grad_y__x_t + eta(t)/2*sum_square_abs(x_sol-x(:,t)));
        % change the norms acording to the problem
        subject to 
        norm(x_sol) <= D_x;
        %norm(x_sol,1) <= D_x;
        %norm(x_sol,inf) <= D_x;
